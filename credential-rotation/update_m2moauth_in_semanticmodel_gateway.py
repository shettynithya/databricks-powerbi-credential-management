...
# MAGIC
# MAGIC 3. The client id , entra secret, databricks secret and tenant ids are saved as secrets in the DBX workspace
...
# MAGIC       tenant_id
# MAGIC
# MAGIC 4. This secret is set in [databricks](https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/oauth-m2m#-step-2-create-an-oauth-secret-for-a-service-principal)  
# MAGIC
...
databricks_client_secret=dbutils.secrets.get(dbutils.widgets.get(""), "")
...
# MAGIC %md
# MAGIC ### Step 1 : Generate credentials for Power BI REST APIs (to authenticate with power bi service)
# MAGIC
...
pbi_svc.update_gateway_m2m_oauth(gatewayId,gateway_datasource_name,encrypted_credentials)
...
pbi_svc.update_dataset_m2m_oauth(workspace_id , dataset_id, client_id,databricks_client_secret)
